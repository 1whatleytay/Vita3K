...
void convert_gxp_to_glsl_from_filepath(const std::string &shader_filepath) {
    const Radical::Path shader_filepath_str{ shader_filepath };
    std::ifstream gxp_stream(shader_filepath_str.get(), std::ifstream::binary);

    if (!gxp_stream.is_open())
        return;

    const auto gxp_file_size = shader_filepath_str.getFileSize();
    ...
    convert_gxp_to_glsl(*gxp_program, shader_filepath_str.getName(), true);
